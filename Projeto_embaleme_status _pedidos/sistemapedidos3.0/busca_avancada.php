...
$codigo = isset($_REQUEST['codigo']) ? $_REQUEST['codigo'] : "1111";

busca_avancada($codigo);
...
function busca_avancada($codigo)
{
  ...
  $sql = "SELECT *,codigo.id as ID_CODIGO FROM codigo INNER JOIN colaborador on codigo.colaborador = colaborador.id where cod = '$codigo' LIMIT 6";
  ...
  if ($result > 0) {
    ...
    echo "<div class='alert alert-success mt-2 text-center' role='alert'>
      <strong> Dados Carregado com Sucesso! </strong>
   </div>";
