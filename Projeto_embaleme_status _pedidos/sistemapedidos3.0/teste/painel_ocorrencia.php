...
$sql = "SELECT chave_nf,data_origem_erro, data_resolucao, datas,status_oc,n_pedido, Concat('R$ ',   
               Replace  
                 (Replace  
                   (Replace  
                     (Format(prejuizo, 2), '.', '|'), ',', '.'), '|', ',')) as prejuizo,id as id_oc,
...
$sql1 = "SELECT Concat('R$ ',   
Replace  
  (Replace  
    (Replace  
      (Format(sum(prejuizo), 2), '.', '|'), ',', '.'), '|', ',')) as prejuizo from ocorrencias";
        ...
        <div id="collapseOne" class="accordion-collapse collapse collapsed" aria-labelledby="headingOne" data-bs-parent="#accordionExample">
            <div class="accordion-body">
                ...
                <form action="busca_avancada.php" method="POST">
                    ...
                    <label for="filtros"> Filtro Adicional: </label>
                    <select name="filtro_adicional" id="filtros">
                        <option value="" selected>Nenhum</option>
                        <option value="1">Maior Prejuizo</option>
                        <option value="2">Mais Erros</option>
                    </select>
                  

                    <label for="data_inicial">Data Inicial <input type="date" name="data_inicial" id="data_inicial"></label>
                    <label for="data_final">Data Final <input type="date" name="data_final" id="data_final"></label>
    ...
    <div class="alert alert-danger d-flex align-items-center" role="alert">
        ...
        <div>
            <?php while ($row = $result1->fetch_assoc()) { ?>
                <strong> Total de Prejuizos R$: <?php echo $row['prejuizo']; ?> Reais.</strong></span>
            <?php } ?></span>
