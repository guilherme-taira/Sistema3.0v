...
class Mostra_dados_php{
    ...
    function getDados(){
        include "conexao.php"; 
        $datanova = date('Y-m-d');
        $sql = "SELECT cod, nome, datas, Tipo, peso, peso_loja,divergencia_peso FROM `codigo` INNER JOIN colaborador on codigo.colaborador = colaborador.id where codigo.colaborador = $this->funcionario and datas like '$datanova%' and Tipo = 'IS'";
        $result = $conn->query($sql);
          ...
          if ($result->num_rows > 0) {
            ...
            echo "Não Há registro da Shopee Digitado";
